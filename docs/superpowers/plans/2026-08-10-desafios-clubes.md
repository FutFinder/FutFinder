...
(`desafio_reglas() ->> 'incomparecencia_horas'`, con su espejo en
`clubChallengeRules.js`), y el índice único es `(match_id, club_reportado_id)`
—uno por partido y por club acusado—, así que los dos clubes pueden acusarse y
cada acusación se revisa por separado.

Tres decisiones que este plan no fijaba y que quedaron tomadas acá:
...
1. **Quién escribe `estado_previo_sancion`.** La transición transversal
   ...
   la sanción. La escribe `solicitar_revision_sancion()`, no la sanción, y sólo
   sobre **el desafío de esa revisión** —los demás siguen protegidos por la
   decisión C3—.
2. **El congelado dura lo que dura la revisión, no lo que dura la sanción.**
   Congelar al aplicar la sanción dejaba atrapado para siempre el encuentro
   cuyo club nunca pide revisión, que es el caso más frecuente. Ahora se congela
   al pedirla y se descongela al resolverla, con cualquiera de las dos
   decisiones y en cuanto no queda ninguna pendiente sobre ese encuentro. Es
   una desviación de la flecha del diagrama, que sólo contemplaba el regreso al
   retirar la sanción.
3. **El hilo queda de solo lectura mientras hay una revisión pendiente.** Es
   consecuencia de lo anterior: `bloqueado_sancion` no está en los
   `estados_activos` de `desafio_reglas()`, que es lo que mira
