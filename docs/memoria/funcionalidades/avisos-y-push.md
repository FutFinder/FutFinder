...
Las preferencias de partidos, clubes, chat y amistades sólo cancelan el push externo: el aviso interno se mantiene. Web y simuladores omiten el registro nativo. La función `send-push` reclama de forma atómica cada aviso, filtra/deduplica tokens, registra tickets por token y deja recibos para el cron; un token marcado definitivamente inválido se limpia. Las categorías incluyen partidos, clubes/desafíos, amistades, mensajes y `chat_mention_all`. El ciclo formal de desafíos agrega `club_challenge_extension`, `club_challenge_closed`, `club_challenge_proposal`, `club_challenge_proposal_rejected` y `club_match_published`, los cinco bajo `notif_clubs`. Los cuatro primeros los emite `desafio_avisar()`, que filtra por `rol = 'admin'`, y llevan `threadKey` al hilo de negociación, salvo cuando el desafío expiró sin que nadie lo aceptara y por lo tanto nunca tuvo hilo. `club_match_published` es la excepción y no pasa por ese helper: lo reciben **todos** los integrantes de los dos clubes, porque el partido publicado es justo lo que un jugador necesita saber para inscribirse, y lleva `matchId` además de `threadKey`. Su destino es el PARTIDO, no el hilo: los jugadores sin rol no tienen nada que hacer en una conversación entre administradores. La tarjeta del aviso ofrece un atajo «VER PARTIDO», declarado en `ATAJOS` junto al dato que necesita — sin ese dato no se dibuja botón. Cualquier tipo nuevo se agrega a la vez en `notificationPreferences.js` y en su espejo `pushLogic.ts`, y necesita destino en `notificationTargets.js`: una prueba compara los tres y falla si uno se queda atrás.
