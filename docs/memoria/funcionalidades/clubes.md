...
**El selector del club activo vive en la cabecera de las seis pestañas** (`ClubHeaderButton`): Inicio, Partidos, Clubes, Reservas, Chat y Perfil. Es un botón del tamaño de la campana —`S.iconBtn`, con el escudo del club activo y el borde de SU tema, que es lo único que dice de un vistazo en nombre de qué club estás mirando la pantalla— y abre una hoja con los clubes, el rol en cada uno y «Explorar clubes». No se dibuja con un solo club, la misma regla que el carrusel, y en el perfil de OTRO jugador tampoco: `PlayerProfileTopBar` es la misma barra para el perfil propio y el ajeno. **El tope son tres clubes y la hoja no ofrece pasarse:** ni lista más de tres —una cuarta membresía no puede existir, la corta el trigger `check_user_club_limit` de la migración 24— ni ofrece «Explorar clubes» a quien ya tiene tres, donde dice «Llegaste al tope de 3 clubes». El número vive en `MAX_CLUBES_POR_JUGADOR` (`services/clubs.js`), que también usan los tres cortes del propio servicio. Toca el mismo `setActiveClub()`, así que la cabecera y el carrusel quedan sincronizados sin que ninguno sepa del otro. La fila de chips es la que NO cabe en una cabecera: mide 42 px de alto y compite con la marca, la billetera y la campana.
