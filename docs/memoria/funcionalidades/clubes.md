...
**El selector del club activo vive en la cabecera de las seis pestañas** (`ClubHeaderButton`): Inicio, Partidos, Clubes, Reservas, Chat y Perfil. Es un botón del tamaño de la campana —`S.iconBtn`, con el escudo del club activo y el borde de SU tema, que es lo único que dice de un vistazo en nombre de qué club estás mirando la pantalla— y abre una hoja con los clubes, el rol en cada uno y «Explorar clubes». No se dibuja con un solo club, la misma regla que los otros dos selectores, y en el perfil de OTRO jugador tampoco: `PlayerProfileTopBar` es la misma barra para el perfil propio y el ajeno. Toca el mismo `setActiveClub()`, así que los tres selectores —cabecera, chips y carrusel— quedan sincronizados sin que ninguno sepa de los otros. La fila de chips es la que NO cabe en una cabecera: mide 42 px de alto y compite con la marca, la billetera y la campana.

**El club activo vale para toda la app, no sólo para la pestaña Clubes.** Inicio lee el mismo `ClubsHomeContext` —ya montado en `MainTabs`, así que no cuesta consultas— y monta su propio `ClubSwitcher` sobre «Mi club» cuando hay más de uno. Antes el club de Inicio salía de `getMyClub()`, que devuelve la membresía MÁS ANTIGUA: con tres clubes se elegía uno en Clubes y en Inicio seguía apareciendo otro, sin manera de cambiarlo desde ahí. De paso desaparece la consulta de permisos delegados que Inicio hacía por su cuenta: `can.responderDesafios` del contexto ya contempla `pubChallenge`/`answerChallenge` (migración 119).
