...
El partido publicado tiene tarjeta propia (`ClubMatchCard`): borde y halo verdes, franja «PARTIDO DE CLUBES», escudos y nombres de los dos clubes con VS, la fecha con más peso y CTA «Ver partido». Aparece en Partidos, en el detalle y en una sección propia de Inicio, «Tu club juega», que es la primera de la pantalla y sólo la ven los integrantes de alguno de los dos clubes. Muestra HASTA TRES partidos por venir —el primero en tarjeta completa y los siguientes compactos—, no sólo el más próximo: quien pertenece a dos clubes tiene dos calendarios. Los pide `listPartidosDeMisClubes()`, club por club a la base, y NO de la tanda general de `listOpenMatches()`: esa trae los N partidos abiertos más próximos de toda la app, así que los partidos ajenos se comían la ventana, y como pide sólo `'abierto'` el partido desaparecía de Inicio al llenarse la nómina. Inicio recarga con `useFocusEffect`, no sólo al montarse: el navegador de pestañas deja la pantalla montada y un partido publicado durante la sesión no llegaba a aparecer. `seleccionInicio()` decide destacados y resto a la vez para que el mismo partido no salga dos veces, y deduplica el partido entre dos clubes míos, que llega dos veces por pedirse por club. Los cupos NO se muestran como compartidos: «9 cupos para tu club» a los integrantes y «9 cupos por club» al resto, siempre desde `cupos_por_club`.
...
**El badge del tile «Desafíos» cuenta lo que hay en su destino.** Contaba los desafíos DIRECTOS recibidos: el número era correcto y el destino también —el tile abre el tablero abierto—, pero la pareja no, porque desde la migración 112 esa pantalla ya no tiene bandeja de directos. Prometía trabajo pendiente y llevaba donde no estaba, el mismo defecto de los hallazgos 1 y 2; además esos desafíos ya tienen su propia tarjeta en «Pendiente para ti». Ahora cuenta las **respuestas que esperan decisión en las publicaciones del club** (`countRespuestasPendientes()`, `services/clubOpenChallenges.js`), que es lo único pendiente que de verdad vive ahí y que hasta ahora sólo se descubría entrando. Sólo `estado = 'pendiente'` y sólo de publicaciones `abierto`: el `respuestasCount` de `listMyOpenChallenges()` cuenta todo lo que no se retiró —aceptadas y rechazadas incluidas— y sirve para rotular «Respuestas (3)» en la tarjeta, no para prometer tres decisiones. Son dos consultas planas y ningún embed, por el mismo motivo que `nominaQuery.js`: un embed mal nombrado no degrada, PostgREST rechaza la consulta entera con 400 y se cae la portada. Sin la migración 112 devuelve 0 y el badge no se dibuja. El rótulo que oye un lector de pantalla también cambió: dice «3 respuestas por decidir», no «3 pendientes».
...
**«Solicitar unirme» en `ClubDetailScreen` ya no se esconde por poder desafiar.** La acción principal bajo la ficha del club elegía UNA sola tarjeta según `soyAdmin` → `puedoDesafiar` → `!soyMiembro && !tengoMaxClubs` → `soyMiembro`, en ese orden — así que quien administraba otro club veía sólo «Desafiar a este club» al mirar uno del que no era miembro, y nunca la opción de pedir entrar, aunque las dos cosas son independientes: se puede administrar un club y a la vez querer sumarse a otro como jugador. Ahora `!soyMiembro && !tengoMaxClubs` (puedo pedir entrar) pasa a decidirse antes que `puedoDesafiar`, y cuando las dos son ciertas se muestran las DOS acciones — «Solicitar unirme»/«Cancelar solicitud» como principal, «Desafiar a este club» como botón secundario debajo—, en vez de que una tape a la otra. El único caso que sigue mostrando sólo «Desafiar» es el que de verdad no puede pedir entrar: ya está en el tope de 3 clubes.
...
**Y el hilo no es para todos, así que el destino depende de quién mira.** Abrirlo exige el permiso `chatClubs` (`chat_puede_ver_desafio`, migración 119 línea 844), de modo que mandar ahí a un jugador que no lo tiene cambia un destino vacío por un muro: el hilo le contesta «este chat es solo para quienes negocian el desafío». `normalizarTareas()` recibe `puedeNegociar` —`rol === 'admin' || chatClubs`, que es literalmente lo que evalúa esa función de la base— y sin él la tarea no lleva `threadKey`: el desafío abre la ficha del club que desafió (y su botón dice «Ver club», porque un «Ver» sobre una ficha ajena no describe nada) y el cambio abre el detalle del partido del que se habla, los dos legibles por cualquier integrante. Por omisión `puedeNegociar` vale lo conservador, `rol === 'admin'`. Lo que NO hizo falta cambiar es la propuesta: `club_challenge_proposals_read` (43:212) y `club_challenges_select` (26:52) dejan leer a cualquier integrante de los dos clubes, así que `ClubProposalScreen` ya era una pantalla de solo lectura correcta para un jugador — simplemente no le dibuja los botones de respuesta.
...
**El cambio de club es inmediato en pantalla, aunque los datos tarden.** `setActiveClub()` apunta al club nuevo en el MISMO gesto, antes de guardar nada y antes de la recarga: hasta la corrección sólo movía `activeClubId` al final de las tres rondas y las catorce consultas, así que tocar otro chip no producía nada visible durante uno o dos segundos y el selector parecía muerto. En el mismo paso viaja la IDENTIDAD del club nuevo —nombre, escudo, comuna, rol, plan, tema—, que ya está en `clubs` porque la trae `getMyClubs()` y no cuesta ninguna consulta. Lo DERIVADO, en cambio, se vacía en vez de heredarse: tareas, badge, próximo partido, actividad, rivales y estadísticas son del club anterior y dejarlos bajo el nombre del nuevo es peor que hacer esperar. `cambiandoClub` marca esa ventana y «Pendiente para ti» dice «Cargando lo de este club…» en lugar de «Todo al día», que con las tareas vacías sería mentira. Ni el selector ni el carrusel se esconden mientras tanto: los dos SON la manera de cambiar de club, y un esqueleto encima dejaría sin salida a quien se equivocó —y al carrusel, además, bajo el dedo que lo desliza.

**El selector del club activo vive en la cabecera de cinco pestañas** (`ClubHeaderButton`): Inicio, Partidos, Clubes, Reservas y Chat — las que comparten la convención de marca a la izquierda y botones de icono a la derecha. Es un botón del tamaño de la campana —`S.iconBtn`, con el escudo del club activo y el borde de SU tema, que es lo único que dice de un vistazo en nombre de qué club estás mirando la pantalla— y abre una hoja con los clubes, el rol en cada uno y «Explorar clubes». No se dibuja con un solo club, la misma regla que el carrusel, y en Perfil NO va: `PlayerProfileTopBar` no es una cabecera de pestaña raíz sino la barra de un detalle, compartida con el perfil de otro jugador. Medido en el navegador a 375 px, sus cinco controles dejan 63 px de título y «Mi perfil» pide 66 —ya estaba a tres píxeles de recortarse—; con un sexto control de 40 px el título quedaba en «M…». Meterlo ahí exige darle dos filas a esa barra, como ya tienen Clubes y Chat. **El tope son tres clubes y la hoja no ofrece pasarse:** ni lista más de tres —una cuarta membresía no puede existir, la corta el trigger `check_user_club_limit` de la migración 24— ni ofrece «Explorar clubes» a quien ya tiene tres, donde dice «Llegaste al tope de 3 clubes». El número vive en `MAX_CLUBES_POR_JUGADOR` (`services/clubs.js`), que también usan los tres cortes del propio servicio. Toca el mismo `setActiveClub()`, así que la cabecera y el carrusel quedan sincronizados sin que ninguno sepa del otro. La fila de chips es la que NO cabe en una cabecera: mide 42 px de alto y compite con la marca, la billetera y la campana.

**El club activo vale para toda la app, no sólo para la pestaña Clubes.** Inicio lee el mismo `ClubsHomeContext` —ya montado en `MainTabs`, así que no cuesta consultas—, y cambiar de club se hace desde el botón de la cabecera: la fila de chips que tuvo sobre «Mi club» se sacó, porque era el mismo control repetido a dos alturas de la misma pantalla. Antes el club de Inicio salía de `getMyClub()`, que devuelve la membresía MÁS ANTIGUA: con tres clubes se elegía uno en Clubes y en Inicio seguía apareciendo otro, sin manera de cambiarlo desde ahí. De paso desaparece la consulta de permisos delegados que Inicio hacía por su cuenta: `can.responderDesafios` del contexto ya contempla `pubChallenge`/`answerChallenge` (migración 119).
...
Del lado del cliente quedaron cuatro reglas escritas donde se pueden probar. **Responder un desafío recibido también se resuelve por acción**: `aceptar_desafio()` autoriza por el permiso `answerChallenge` en el club RETADO (119:666) y `club_challenges_update` (119:575) deja rechazar igual, pero `getChallengeCta` preguntaba sólo por el rol y devolvía «Solo lectura» antes de mirar el estado — el servidor decía que sí y la interfaz que no, el mismo defecto que el hallazgo 5 nombró en la nota de los accesos rápidos. Ahora `challengeCtaContext` recibe `misClubIdsResponder` y lo filtra contra el club retado —tenerlo en el club que retó no habilita nada, y ofrecer el botón ahí sería mandar al usuario contra un 42501—, y el permiso abre el estado `pendiente` y nada más: en negociación sigue siendo lectura. Sale de la MISMA consulta de permisos que ya hacía el hilo para `results`, así que no cuesta un viaje más. **El permiso de resultados se resuelve por acción y no por rol**: el hilo del desafío preguntaba «¿soy administrador?» y devolvía «Solo lectura» antes de mirar el estado, así que al delegado de `results` no le aparecía ni registrar ni confirmar, que es justo lo que el servidor sí le autoriza; ahora `challengeCtaContext` recibe los clubes donde tengo ese permiso y `getChallengeCta` lo mira sólo en los estados del resultado. **El tablero de desafíos conserva dos booleanos**, uno por `pubChallenge` y otro por `answerChallenge`, en vez de juntarlos en uno que habilitaba acciones distintas. Y **la alineación cuenta con la nómina de ahora**: `asignacionesVigentes()` depura los `member_id` que ya no están en el club, así que un expulsado deja de contar como puesto ocupado y su lugar vuelve a estar libre para autocompletar.
