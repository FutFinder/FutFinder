...
El héroe de Portada (`WelcomeScreen`) arranca ocupando toda la pantalla (logo, titular, subtítulo y un hint «DESLIZA») y se contrae a medida que se scrollea, revelando debajo el resto (avatares, ticker, features y el botón «Siguiente») — animado con `Animated.ScrollView` sobre un `scrollY` compartido, sin `expo-reanimated`. La distancia de scroll para terminar de contraerse es fija (`SCROLL_RANGE`, 296px); como el contenido revelado no siempre alcanza esa distancia por sí solo (depende de fuente cargada, ancho de pantalla), un `onLayout` mide su alto real y un relleno al final del scroll (`bottomPad`) garantiza que siempre haya scroll suficiente — si no, el héroe se queda pegado a mitad de camino, tapando el contenido en vez de revelarlo.
