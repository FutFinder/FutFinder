...
**Tocar `_shared/` no cambia nada en producción hasta redesplegar la función que lo importa.** Las Edge Functions se despliegan con su copia de `_shared/` adentro, así que un commit que sólo toca `_shared/correoSolicitud.ts` deja el repo y el servidor diciendo cosas distintas. Pasó el 2026-09-18 con `DESTINO_POR_OMISION`: el commit estaba en `main` y la función desplegada seguía siendo la v4, sin el destino por omisión. Se redesplegó (v5, `solicitud-recinto/index.ts` + `_shared/correoSolicitud.ts`) y ahí recién quedó vivo. Al cambiar algo de `_shared/`, redesplegar TODAS las funciones que lo importan.
