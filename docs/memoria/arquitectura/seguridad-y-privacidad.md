...
- La dirección exacta de un partido de clubes se reserva a los integrantes de los dos clubes también al PINTAR. La RLS de `club_challenge_proposals` la protege mientras es propuesta, pero al publicarse el partido pasa a `matches`, que es de lectura pública: la reserva no sobrevive sola. `lugarLabel()` entrega cancha y comuna a cualquiera —hacen falta para saber si queda cerca— y la calle sólo a los dos clubes, y «Cómo llegar» se apaga para el resto, porque abrir el mapa en el punto exacto es enseñar la dirección por otra puerta. Es una defensa de interfaz: `matches.direccion`, `latitud` y `longitud` siguen siendo legibles por cualquiera vía PostgREST.
