...
- **Los permisos del club mueren con la membresía (migración 129, reproducido en producción el 2026-09-21):** `tiene_permiso_de_club` resolvía la excepción individual sin exigir membresía, y `club_member_permission_overrides` cuelga del club y de `auth.users`, no de `club_members`. Un expulsado con `editClub` delegado seguía editando el club. Ahora el ayudante devuelve `false` de entrada si la persona no está en `club_members` —eso protege a todas las políticas y RPC que lo usan— y un trigger borra sus excepciones al terminar la membresía, para que una reincorporación no las resucite en silencio.
- **Una política autoriza la FILA, no las columnas (migración 129).** `clubs_update` deja editar al que tiene `editClub`, pero `authenticated` tenía UPDATE sobre TODAS las columnas de `clubs`: un administrador se ponía `plan = 'premium'` y `verificado = true`. Y en `club_join_requests` tenía UPDATE sobre `club_id`, así que el invitado del club A apuntaba su invitación al club B, la aceptaba y el trigger le creaba la membresía en B. Los dos se cerraron con privilegios POR COLUMNA: en `clubs` se conceden sólo las que escribe el formulario, el logo y el banner; en `club_join_requests`, `status` y `responded_at` para responder y `club_id`/`user_id`/`tipo` sólo al crearla. Ninguna función del esquema escribe esas dos tablas, así que los privilegios son la puerta completa.
