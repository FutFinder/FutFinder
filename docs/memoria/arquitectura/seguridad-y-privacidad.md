...
- La ubicación exacta de un partido de clubes está protegida EN LA BASE, no en la interfaz. Vivía en `matches`, cuya política de lectura es `using (true)`, así que cualquiera —incluido `anon`— la leía por PostgREST; y `tg_register_cancha` la copiaba además a la tabla pública `canchas`, consultable con `search_canchas()`. Desde la 44b la exacta vive en `club_match_locations` con RLS para los integrantes de los dos clubes, y `matches` conserva sólo un punto aproximado —rejilla de 0,01°, ~1 km— marcado con `ubicacion_aproximada`, sin calle. Se separó en vez de cerrar la lectura de `matches` porque cerrarla habría roto el listado de partidos abiertos, que es media app.
- La distancia que ve alguien de fuera es aproximada y puede errar hasta ~0,73 km. La interfaz lo dice («Ubicación aproximada», «A unos X km») y «Cómo llegar» sólo se habilita con la exacta. El GPS de asistencia usa exclusivamente la exacta y falla cerrado sin ella: confirmar contra un punto redondeado a un kilómetro dejaría marcarse presente desde una cuadra.
- El NOMBRE DE LA CANCHA sigue siendo público, junto con comuna, región, fecha y cupos: es lo que hace falta para descubrir el partido. Si el recinto es conocido, su nombre identifica el lugar mejor que la celda de 1 km. La aproximación protege la coordenada, no el nombre.
