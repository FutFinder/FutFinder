          ...
          return (
            <Pressable
              ...
              className={`flex-row items-center gap-1.5 rounded-full border px-3 py-1.5 ${
                on ? 'border-verde/40 bg-verde/16' : 'border-white/12 bg-black/50'
              }`}
            >
              <Text className={`text-[10.5px] font-bold tracking-[0.14em] ${on ? 'text-verde' : 'text-white/60'}`}>
                {c.label}
              </Text>
              <View className={`min-w-[17px] items-center justify-center rounded-full px-1 py-px ${on ? 'bg-verde' : 'bg-white/10'}`}>
                <Text className={`text-[10px] font-bold ${on ? 'text-verde-ink' : 'text-white/60'}`}>{c.count}</Text>
              </View>
            </Pressable>
          );
        })}
